...
app.whenReady().then(() => {
  ...
  // Handler for generating AI document summaries
  ipcMain.handle('generate-document-summaries', async (event, data) => {
    try {
      console.log(`[main.js] Generating AI summaries for: ${data.filePath}`);
      
      if (!data.content || data.content.trim().length === 0) {
        return { success: false, error: 'No content provided' };
      }

      // For now, implement a simple fallback until AI integration is added
      ...
      // Extract first sentence for one-line summary
      const sentences = content.split(/[.!?]+/).filter(s => s.trim().length > 20);
      let summarySentence = '';
      
      if (sentences.length > 0) {
        summarySentence = sentences[0].trim() + '.';
      } else {
        summarySentence = content.substring(0, 100).trim() + '...';
      }
      
      console.log(`[main.js] Generated summaries - Paragraph: ${summaryParagraph.length} chars, Sentence: ${summarySentence.length} chars`);
      
      return {
        success: true,
        paragraph: summaryParagraph,
        sentence: summarySentence,
        method: 'fallback' // Indicates this was generated by fallback, not AI
      };
      
    } catch (error) {
      console.error('[main.js] Error generating summaries:', error);
      return { success: false, error: error.message };
    }
  });
...
ipcMain.handle('write-file', async (event, filePath, content) => {
  ...
  // AI handlers moved to ipc/aiHandlers.js (16 handlers)
...
<body>
  ...
  // Navigation handlers moved to ipc/navigationHandlers.js
