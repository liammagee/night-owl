...
// Removed unused writeFile import - now using fs.writeFile directly in saveBinaryFile
...
async function saveBinaryFile(fileName, content) {
  ...
  try {
    ...
    // Save file in the generated-images directory
    const filePath = path.join(imagesDir, fileName);
    await fs.writeFile(filePath, content);
    console.log(`File ${fileName} saved to generated-images directory.`);
  } catch (err) {
...
class GeminiProvider extends BaseProvider {
  ...
  async sendMessage(message, options) {
    const {
      model = process.env.GOOGLE_MODEL || 'gemini-2.5-flash-image-preview',
      systemMessage,
      temperature = parseFloat(process.env.AI_TEMPERATURE) || 0.7,
      maxTokens = parseInt(process.env.AI_MAX_TOKENS) || 2000,
      conversationHistory = [],
    ...
    try {
      ...
      let contents = this.buildHistory(conversationHistory, systemMessage);
      contents.push({
          role: 'user',
          parts: parts,
        });
      ...
      let fileIndex = 0;
      const generatedImages = []; // Array to collect generated images
      
      for await (const chunk of response) {
        // console.log('[GeminiProvider] 🔍 DEBUG: Raw chunk received:', JSON.stringify(chunk, null, 2));
        
        if (!chunk.candidates || !chunk.candidates[0].content || !chunk.candidates[0].content.parts) {
          console.log('[GeminiProvider] 🔍 DEBUG: Skipping chunk - missing candidates/content/parts');
          continue;
        }
        ...
        for (let i = 0; i < parts.length; i++) {
          const part = parts[i];
          // console.log(`[GeminiProvider] 🔍 DEBUG: Part ${i}:`, part);
          
          if (part.inlineData) {
            console.log('[GeminiProvider] 🖼️ DEBUG: Found inlineData! Processing image...');
            foundImage = true;
            const timestamp = new Date().toISOString().replace(/[:.]/g, '-').slice(0, -5); // Format: YYYY-MM-DDTHH-MM-SS
            const fileName = `generated_image_${timestamp}_${fileIndex++}`;
            const inlineData = part.inlineData;
            const fileExtension = mime.getExtension(inlineData.mimeType || 'image/png');
            const buffer = Buffer.from(inlineData.data || '', 'base64');
            
            // Save to file (existing functionality) - don't await as it's not needed for chat display
            saveBinaryFile(`${fileName}.${fileExtension}`, buffer).catch(err => 
              console.error('[GeminiProvider] File save error:', err)
            );
            
            // Add to images array for chat display
            ...
            console.log(`[GeminiProvider] 🖼️ Generated image: ${fileName}.${fileExtension}`);
          }
        }
        
        if (!foundImage) {
          console.log('[GeminiProvider] 🔍 DEBUG: Text chunk received:', chunk.text);
          if (chunk.text) {
            text += chunk.text;
          }
        }
      ...
      if (generatedImages.length > 0) {
        console.log(`[GeminiProvider] 🖼️ Generated ${generatedImages.length} image(s) for chat display`);
      }

      console.log(`[GeminiProvider] 🔍 Final response - images count: ${generatedImages.length}`);
      console.log(`[GeminiProvider] 🔍 Final response - text length: ${text.length}`);
      if (generatedImages.length > 0) {
        console.log(`[GeminiProvider] 🔍 First image preview:`, {
          mimeType: generatedImages[0].mimeType,
          filename: generatedImages[0].filename,
          base64Length: generatedImages[0].base64?.length || 0
        });
      }
