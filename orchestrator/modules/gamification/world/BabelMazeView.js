...
class BabelMazeView {
    ...
    async cmdEdit() {
        ...
        if (!window?.electronAPI?.invoke || typeof window.openFileInEditor !== 'function') {
            this.appendLog('system', 'Editor bridge unavailable in this environment.');
            return;
        }

        try {
            window.switchToMode?.('editor');
            const result = await window.electronAPI.invoke('open-file-path', room.path);
            if (!result?.success) {
            ...
            console.error('[BabelMazeView] Edit failed:', error);
    ...
    async cmdLink(rawTarget) {
        ...
        try {
            ...
            console.error('[BabelMazeView] Link failed:', error);
    ...
    async tryInsertLinkInOpenEditor(sourcePath, targetNode) {
        ...
        const currentPath = this.normalizeAbsPath(window.currentFilePath || '');
        ...
        const saveResult = await window.electronAPI.invoke('perform-save-with-path', newContent, sourceAbs);
    ...
    async cmdWhisper(question) {
        ...
        const companion = this.gamification?.aiCompanion || window.aiCompanion || window.aiCompanionManager;
...
Respond in 2–5 sentences: atmospheric but actionable. Suggest 1 concrete next move (e.g., link X to Y, write a 2-sentence abstract, rename a note, create a hub note).
    ...
    async buildGraph() {
        if (!window?.getFilteredVisualizationFiles || !window?.electronAPI?.invoke) {
        ...
        for (const record of records) {
            const contentResult = await window.electronAPI.invoke('read-file-content-only', record.path);
