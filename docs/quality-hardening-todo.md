...
- Full Jest suite currently passes after the latest hardening pass: 68 suites, 1,097 tests.
...
- [x] Add event-level regression coverage for moving multiple selected files through file-tree drag/drop.
...
- [x] Replace remaining non-file-tree `confirm()` flows in settings, git, citations, formatting, Kanban, tab dirty-close paths, PDF annotation save choices, and presentation recording.
...
- [x] Add a static guard that rejects raw browser `confirm()` calls in app/plugin JavaScript.
