...
const Upload = () => /*#__PURE__*/React.createElement("svg", {
  width: "16",
  height: "16",
  viewBox: "0 0 24 24",
  fill: "none",
...
const ZoomIn = () => /*#__PURE__*/React.createElement("svg", {
  width: "16",
  height: "16",
  viewBox: "0 0 24 24",
  fill: "none",
...
const ZoomOut = () => /*#__PURE__*/React.createElement("svg", {
  width: "16",
  height: "16",
  viewBox: "0 0 24 24",
  fill: "none",
...
const StickyNote = () => /*#__PURE__*/React.createElement("svg", {
  width: "16",
  height: "16",
  viewBox: "0 0 24 24",
  fill: "none",
...
const Speaker = () => /*#__PURE__*/React.createElement("svg", {
  width: "16",
  height: "16",
  viewBox: "0 0 24 24",
  fill: "none",
...
const SpeakerOff = () => /*#__PURE__*/React.createElement("svg", {
  width: "16",
  height: "16",
  viewBox: "0 0 24 24",
  fill: "none",
...
const RecordIcon = () => /*#__PURE__*/React.createElement("svg", {
  width: "16",
  height: "16",
  viewBox: "0 0 24 24",
  fill: "currentColor",
...
const StopIcon = () => /*#__PURE__*/React.createElement("svg", {
  width: "16",
  height: "16",
  viewBox: "0 0 24 24",
  fill: "currentColor",
...
\`\`\``;
  ...
  const processListSection = listMarkdown => {
    ...
    if (window.marked && typeof window.marked.parse === 'function') {
      try {
        let parsedHtml = window.marked.parse(listMarkdown, {
          breaks: false
        });
    ...
    if (slides.length > 0 && window.MathJax && window.MathJax.typesetPromise) {
      ...
      const timer = setTimeout(() => {
        ...
        if (presentationContainer) {
          ...
          window.MathJax.typesetPromise([presentationContainer]).then(() => console.log('[MathJax] Math rendering completed successfully')).catch(err => console.error('[MathJax] Error rendering math in presentation:', err));
        } else {
  ...
  const resetView = () => {
    ...
    const targetZoom = 1;
    const centeredPan = computeCenteredPan(slides[baseSlideIndex], targetZoom, {
      x: 0,
      y: 0
    });
    markZoomInteraction();
  ...
  useEffect(() => {
    ...
    const handleWheel = e => {
      ...
      let targetZoom = normalizedDelta < 0 ? previousZoom * zoomFactor : previousZoom / zoomFactor;
  ...
  useEffect(() => {
    ...
    if (ttsEnabled && slides[currentSlide]?.speakerNotes) {
      ...
      setTimeout(() => {
        const currentTtsEnabled = ttsStateRef.current.ttsEnabled !== undefined ? ttsStateRef.current.ttsEnabled : ttsEnabled;
    ...
    if (window.ttsService) {
      try {
        ...
        const handleCompletion = () => {
          ...
          const currentTtsEnabled = ttsStateRef.current.ttsEnabled !== undefined ? ttsStateRef.current.ttsEnabled : true; // Assume true if not set, since we're in a completion callback
          ...
          if (currentTtsEnabled && slideIndex < slides.length - 1) {
            ...
            setTimeout(() => {
              // Get current ttsEnabled state to avoid closure issues
              const currentTtsEnabled = ttsStateRef.current.ttsEnabled !== undefined ? ttsStateRef.current.ttsEnabled : ttsEnabled; // fallback to state if ref not set
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    className: `w-2 h-2 rounded-full transition-colors ${index === currentSlide ? 'bg-green-500' : 'bg-gray-400'}`
  })))), /*#__PURE__*/React.createElement("button", {
    onClick: () => goToSlide(currentSlide + 1),
    ...
    className: `p-2 rounded-lg transition-colors shadow-lg border ${speakerNotesWindowVisible ? 'bg-green-600 hover:bg-green-700 text-white border-green-700' : 'bg-cream hover:bg-gray-100 text-gray-900'}`,
    ...
    className: `p-2 rounded-lg transition-colors shadow-lg border ${ttsEnabled ? 'bg-blue-600 hover:bg-blue-700 text-white border-blue-700' : 'bg-cream hover:bg-gray-100 text-gray-900'}`,
    ...
    return /*#__PURE__*/React.createElement("div", {
      key: slide.id,
      className: `absolute slide rounded-xl shadow-2xl transition-all duration-500 cursor-pointer transform ${isFocused ? 'ring-4 ring-purple-500 shadow-purple-500/50 animate-pulse' : isCurrent ? 'ring-4 ring-green-500 shadow-green-500/50 scale-105' : 'hover:shadow-3xl hover:scale-105 hover:ring-2 hover:ring-blue-400'}`,
      style: {
